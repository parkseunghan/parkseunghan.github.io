---
title: "사이버 공격 유형 예측을 위한 머신러닝 모델 비교"
categories:
  - Machine Learning
tags:
  - Machine Learning
  ...
  - Classification
  - Model Comparison
last_modified_at: 2024-12-11T14:01:00+09:00
published: true
---

## 요약

사이버 보안 공격 데이터셋을 기반으로 `KNN`, `Random Forest`, `SVM`, `DNN`을 비교한 프로젝트임

공격 유형은 `Malware`, `DDoS`, `Intrusion` 3개 클래스로 구성되며, 목표는 전처리 방식과 모델별 특성을 비교해 어떤 한계가 있는지 확인하는 데 있음

결론적으로는 네 모델 모두 유의미한 성능 향상을 보이지 않았고, 데이터셋 자체의 한계와 특성 구성 문제가 큰 영향을 준 것으로 정리됨
